...
export function SupplyStatCard({ material, received, inTransit }: SupplyStatCardProps) {
  return (
    <div className="bg-surface border border-base rounded-md p-[18px] px-5 flex gap-3.5 items-start shadow-card">
      ...
      <div className="flex flex-col gap-0.5 min-w-0">
        ...
        <div className="text-[24px] font-bold text-ink tracking-[-0.02em] tabular-nums">
          {fmtNum(received)}{' '}
          <span className="text-[13px] font-medium text-muted">kg total</span>
        </div>
        <div className="mt-1.5 text-xs text-muted inline-flex items-center gap-1.5">
          {inTransit > 0 ? (
            <>
